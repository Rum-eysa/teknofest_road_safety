# -*- coding: utf-8 -*-
"""
Teknofest Road Safety - Otomatik Kod Duzeltme Scripti v2
...
def check_directory():
    print("\n" + "="*70)
    print("  Teknofest Road Safety - Otomatik Kod Duzeltme Scripti v2")
    print("="*70 + "\n")
    ...
    if not predict_path.exists():
        ...
        print(f"       Aranan dosya: {predict_path.absolute()}")
        sys.exit(1)
    
    print(f"[OK] Calisma dizini dogru: {os.getcwd()}\n")


def create_requirements_txt():
    """requirements.txt dosyasini olustur"""
    print("[0/6] requirements.txt olusturuluyor...")
    
    requirements = """loguru==0.7.0
...
"""
    ...
    try:
        ...
        print("[OK] requirements.txt olusturuldu\n")
    except Exception as e:
        print(f"[UYARI] requirements.txt olusturulamadi: {e}\n")


def create_gitattributes():
    """Line ending sorununu cozmek icin .gitattributes olustur"""
    print("[0.5/6] .gitattributes olusturuluyor...")
...
"""
    ...
    try:
        ...
        print("[OK] .gitattributes olusturuldu\n")
    except Exception as e:
        print(f"[UYARI] .gitattributes olusturulamadi: {e}\n")
...
'''
    ...
    try:
        ...
        print(f'[HATA] src/utils.py guncelleme basarısız: {e}\n')
...
'''
    ...
    try:
        ...
        print(f'[HATA] src/predict.py guncelleme basarısız: {e}\n')


def update_yaml_configs():
    """YAML dosyaları daha robust pattern ile guncelle"""
    print("[3/6] Model A konfigurasyonlari guncelleniyor...")
    files_a = [
...
'''
    ...
    # Daha robust pattern (tirmaksiz, whitespace esnek)
    pattern_a = r'classes:\s*\n(?:\s*-\s*\w+.*\n)+'
    
    for fpath in files_a:
        _modify_yaml(fpath, pattern_a, new_classes_a)

    print("\n[4/6] Model B konfigurasyonlari guncelleniyor...")
    files_b = [
...
def _modify_yaml(fpath, pattern, replacement):
    """YAML dosyasini daha secure ve robust şekilde degistir"""
    fpath = Path(fpath)
    ...
    try:
        ...
        if re.search(pattern, content, re.MULTILINE):
            ...
            if 'classes:' in content:
                print(f"     (Dosya 'classes:' içeriyor, format farkli olabilir)")
    except Exception as e:
        print(f"[HATA] {fpath} islenirken hata: {e}")
...
def verify_changes():
    ...
    try:
        ...
        print('\nINFERENCE_SKIP_LABELS:')
        print(f'  {INFERENCE_SKIP_LABELS}')
        
    except ImportError as e:
        print(f"[UYARI] Import dogrulamasi calismadi: {e}")
        print("         (Gerekli kütüphaneler yerelde kurulu olmayabilir.)")
        print("         Docker'da sorunsuz çalışacaktır.")
    except Exception as e:
        print(f"[HATA] Dogrulama sirasinda hata: {e}")
...
def git_status():
    ...
    try:
        ...
        print('  git add -A')
        print('  git commit -m "fix: encoding, patterns, requirements - v2"')
        print('  git push')
    except Exception as e:
        print(f"[UYARI] Git komutu calistirilamadi: {e}")
...
def show_summary():
    ...
    print("    + loguru, opencv-python, torch, ultralytics\n")
    
    print(" .gitattributes")
    print("    + Line ending (CRLF/LF) problemi cozumlendi\n")
    
    print(" src/utils.py")
    print("    + MODEL_A_YOLO_CLASSES: Roboflow alfabetik sirasi")
    print("    + MODEL_B_YOLO_CLASSES: kemer_takili=7 eklendi")
    print("    + VALID_DRIVER_ACTIONS: slalom eklendi\n")
    
    print(" src/predict.py")
    print("    + vehicle_type==plaka oldugunda atlanıyor (sedan degil)")
    print("    + kemer_takili ve slalom filtreleme: INFERENCE_SKIP_LABELS")
    print("    + Majority voting: tum video en guvenilir arac_bilgisi")
    print("    + Frame skip=5: T4 timeout onleme\n")
    
    print(" Konfigürasyon Dosyaları")
    print("    + model_a ve model_b YAML dosyaları Roboflow export sirası ile senkronize\n")
    ...
    print(" 2. data.yaml icindeki 'names:' sirasi ile config siniflarini dogrula")
    print(" 3. Bağımlılıkları kur: pip install -r requirements.txt")
    print(" 4. Docker build: docker compose build baseline")
    print(" 5. Ağırlıkları kopyala: cp output/experiment_baseline/weights/best.pt models/best_a.pt\n")
...
def main():
    try:
        ...
        print("   Repository'nize 'git push' yapabilirsiniz.\n")
